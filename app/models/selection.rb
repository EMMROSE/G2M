  ...
  def generate_csv
    ...
    CSV.generate(csv_options) do |csv|
      ...
      self.products.each do |product|
        ...
        csv << ["#{product.name}-#{product.id}","#{product.name} #{product.color}",'<meta content="text/html charset=utf-8"" http-equiv=""content-type""><span data-mce-fragment=""1"">Occasion - excellent état<br>Cet article est unique.1 seul exemplaire est en stock.</span>',"#{product.brand}","#{product.category}","#{product.genre}","true","Taille","#{product.size}","","","","","#{product.id}","0.0","shopify",1,"deny","manual","#{product.price}","","true","true","","#{image_url}",1,"","false","","","","","","","","","","","","","","","","#{image_url}","kg","",""]
